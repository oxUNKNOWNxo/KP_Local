...
# iOS menu responsiveness: Program.charge() performs synchronous cache
# destruction, Resources.UnloadUnusedAssets(), and GC.Collect() every five
# minutes. Any navigation target that calls charge() can therefore stall the
# main thread. iOS already has low-memory/cache maintenance hooks, so skip this
# aggressive synchronous cleanup on device.
charge_match = re.search(r"(?m)^    public static void charge\(\)\s*\{", text)
if not charge_match:
    raise SystemExit("Could not locate Program.charge()")
charge_open = text.find("{", charge_match.start())
depth = 0
charge_close = -1
for i in range(charge_open, len(text)):
    if text[i] == "{":
        depth += 1
    elif text[i] == "}":
        depth -= 1
        if depth == 0:
            ...
            break
if charge_close < 0:
    raise SystemExit("Unbalanced Program.charge() body")
charge_body = text[charge_open + 1:charge_close]
if "Resources.UnloadUnusedAssets();" not in charge_body or "GC.Collect();" not in charge_body:
    raise SystemExit("Unexpected Program.charge() implementation")
charge_replacement = """{
#if UNITY_IOS || UNITY_IPHONE
        // Avoid synchronous navigation stalls on iOS. Cache maintenance and
        // low-memory cleanup continue through the normal runtime paths.
        return;
#else""" + charge_body + """
#endif
    }"""
text = text[:charge_open] + charge_replacement + text[charge_close + 1:]
...
print("  - disabled synchronous Program.charge cleanup on iOS")
print("  - left explicit/manual Resource Update behavior unchanged")

# ---------------------------------------------------------------------------
# Make iOS servant/menu transitions immediate.
# ---------------------------------------------------------------------------
servant_path = assets / "SibylSystem" / "Servant.cs"
if not servant_path.is_file():
    raise SystemExit(f"Servant source not found: {servant_path}")
servant_text = servant_path.read_text(encoding="utf-8-sig")

servant_show_old = """    public virtual void show()
    {
        if (isShowed == false)
        {
            isShowed = true;
...
servant_show_new = """    public virtual void show()
    {
        if (isShowed == false)
        {
            isShowed = true;
...
if servant_show_old not in servant_text:
    raise SystemExit("Could not locate Servant.show() transition")
servant_text = servant_text.replace(servant_show_old, servant_show_new, 1)

servant_hide_old = """    public virtual void hide()
    {
        RMSshow_clear();
        RMSshow_clearYNF();
...
servant_hide_new = """    public virtual void hide()
    {
        RMSshow_clear();
        RMSshow_clearYNF();
...
if servant_hide_old not in servant_text:
    raise SystemExit("Could not locate Servant.hide() transition")
servant_text = servant_text.replace(servant_hide_old, servant_hide_new, 1)

domove_old = "            toolBar.transform.DOMove(targetWorldPosition, 0.6f);"
domove_new = """#if UNITY_IOS || UNITY_IPHONE
            toolBar.transform.position = targetWorldPosition;
#else
            toolBar.transform.DOMove(targetWorldPosition, 0.6f);
#endif"""
domove_count = servant_text.count(domove_old)
if domove_count != 2:
    raise SystemExit(f"Expected two toolbar DOMove transitions, found {domove_count}")
servant_text = servant_text.replace(domove_old, domove_new)

servant_path.write_text(servant_text, encoding="utf-8")
print(f"Patched: {servant_path}")
print("  - made iOS Servant show/hide layout immediate")
print("  - removed 0.6s toolbar tween from iOS navigation")
