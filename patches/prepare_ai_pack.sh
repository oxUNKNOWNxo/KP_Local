...
call_match = re.search(r'(?m)^[ \t]*EnableAiMenuEntry\(\);[ \t]*PY
...
registration_matches = list(re.finditer(
    r'(?m)^[ \t]*UIHelper\.registEvent\(gameObject,\s*"ai_",\s*onClickAI\);[ \t]*PY
...
if len(registration_matches) != 1:
    raise SystemExit(f'Expected exactly one active ai_ event registration; found {len(registration_matches)}')
if call_match is None or call_match.start() > registration_matches[0].start():
